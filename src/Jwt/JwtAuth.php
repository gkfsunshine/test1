...
class JwtAuth
{
    ...
    public function check($request, $token = '')
    {
        ...
        //取消拼接
        /*$get_prefix = substr($token, 0, 7);
        if ($get_prefix == 'jialeo_') {
            $session_key = \Crypt::decrypt(substr($token, 7));
            if (!$session_key) {
                return false;
            }
    ...
    public function createToken(array $session_data = array(), $device = '')
    {
        ...
        \Cache::add(self::$sessionPrefix . ':' . $session_key, $session_data, $session_expires_time);
        return [

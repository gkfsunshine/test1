...
class AdvertisingCommand extends Command
{
    ...
    public function handle()
    {
        ...
        // insertData
        $route_id = 1000001;
        if ($this->insertData($route_id)) {
            $this->info('插入必需数据成功!');
        }

        //route
        $this->warn(str_repeat('*', 15));
        $this->warn("请手动将 Route::resource('banners', 'Admin\AdvertisingController',['as' => 'name:拓展管理权限组:广告|menu:拓展管理:广告列表|route_id:" . $route_id . "']); //广告管理 添加到route/admin.php");
        $this->warn("请手动将 Route::get('banners','Api\AdvertisingController@index'); //广告列表 添加到route/api.php");
        $this->warn("请到 Admin/UplaodController 添加一个新的upload_type为banners，确认upload配置无误后再开始使用');");
     ...
     * @return bool
     */
    private function insertData($route_id)
    {
        try {
            \DB::beginTransaction();
            // 插入菜单数据

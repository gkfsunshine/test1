...
class WechatOrigin
{
    ...
    const QIYE_MEDIA_URL = '/media/get?';
...
//		return '{' . $json . '}'; //Return associative JSON
    ...
    public function getQiyeJsTicket($appid = '', $jsapi_ticket = '')
    {
        ...
        $result = $this->http_get(self::API_QIYE_URL_PREFIX . self::GET_QIYE_TICKET_URL . 'access_token=' . $this->access_token['access_token']);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array
     */
    public function getQiYeMedia($media_id)
    {
        if (!$this->access_token && !$this->checkQiyeAuth()) return false;
        $result = $this->http_get(self::API_QIYE_URL_PREFIX . self::QIYE_MEDIA_URL . 'access_token=' . $this->access_token['access_token'].'&media_id='.$media_id);
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function sendQiYeMessage($data)
    {
        ...
        $result = $this->http_post(self::API_QIYE_URL_PREFIX . self::QIYE_MESSAGE_SEND_URL . 'access_token=' . $this->access_token['access_token'], self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
